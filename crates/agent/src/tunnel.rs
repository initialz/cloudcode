...
pub enum ServerMsg {
    ...
    PtyOpen {
        ...
        #[serde(default)]
        sandbox: bool,
        /// Which tool to launch in the first pane. As of v1.13 this is
        /// effectively claude-only; `None` lets the agent fall back to
        /// its `[tools].default`. Kept on the wire for back-compat with
        /// pre-v1.10 hubs/clients that didn't know about multi-tool.
        #[serde(default)]
        tool: Option<String>,
    },
    PtyResize {
